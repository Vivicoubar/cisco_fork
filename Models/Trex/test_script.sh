...
docker run --rm -v $(pwd)/../../DBs/Dataset-Muaz/pairs/:/pairs \
	-v $(pwd)/Preprocessing/Dataset-Muaz-trex/:/traces -v $(pwd)/NeuralNetwork/:/output -it trex-inference conda run --no-capture-output -n trex python3 trex_inference.py --input-pairs /pairs/pairs_testing_Dataset-Muaz.csv --input-traces /traces/trex_traces.json --model-checkpoint-dir checkpoints/similarity/ --data-bin-dir data-bin-sim/similarity/ --output-dir /output/Dataset-Muaz-trex

cp NeuralNetwork/Dataset-Muaz-trex/pairs_testing_Dataset-Muaz.csv.trex_out.csv ../../Results/csv/pairs_results_Dataset-Muaz_trx.csv
